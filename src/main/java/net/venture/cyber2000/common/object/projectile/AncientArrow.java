package net.venture.cyber2000.common.object.projectile;
...
public class AncientArrow extends AbstractArrow {
    ...
    public AncientArrow(Level level, double pX, double pY, double pZ) {
        super(VentureEntityTypes.ANCIENT_ARROW.get(), pX, pY, pZ, level);
    }

    public AncientArrow(Level level, LivingEntity shooter) {
        super(VentureEntityTypes.ANCIENT_ARROW.get(), shooter, level);
    }
    ...
    private void makeParticle(int particleAmount) {
        ...
        if (particleAmount > 0) {
            ...
            for(int j = 0; j < particleAmount * 5; ++j) {
                if ((j % 5) == 1) {
                    this.level().addParticle((ParticleOptions) VentureParticles.ANCIENT_STAR.get(), this.getRandomX(0.5D), this.getRandomY(), this.getRandomZ(0.5D), d0, d1, d2);
                }
            }
        }
    }
